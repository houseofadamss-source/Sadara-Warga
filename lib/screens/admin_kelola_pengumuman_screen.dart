...
class _AdminKelolaPengumumanScreenState extends State<AdminKelolaPengumumanScreen> {
  ...
  Future<void> _deleteAnnouncement(String id) async {
    // 1. Simpan salinan sementara buat balikin data kalau gagal
    final backup = List<Map<String, dynamic>>.from(_announcements);
    
    // 2. Optimistic Update (Hapus dari UI Admin)
    setState(() {
      _announcements.removeWhere((item) => item['id'].toString() == id);
    });

    try {
      // 3. Kirim Broadcast ke Beranda biar dia hapus juga
      final channel = Supabase.instance.client.channel('global_sync');
      ...
      if (mounted) {
        setState(() => _announcements = backup); // Balikin data
        ScaffoldMessenger.of(context).showSnackBar(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Geser/Slide ke kiri pada kartu pengumuman untuk menghapusnya secara permanen dari sistem.',
                  ...
                  sliver: SliverList(
                    delegate: SliverChildBuilderDelegate(
                      (context, index) {
                        ...
                        final String date = DateFormat('EEEE, d MMM, HH:mm').format(DateTime.parse(item['created_at']).toLocal());
                                      ...
                                      children: [
                                        Row(
                                          ...
                                          children: [
                                            ...
                                            Text(DateFormat('d MMM').format(DateTime.parse(item['created_at']).toLocal()), style: const TextStyle(fontSize: 10, color: Color(0xFF94A3B8))),
                                          ],
                                        ),
